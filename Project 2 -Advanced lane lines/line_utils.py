...
class Line:
    ...
    def __init__(self, buffer_len = 10):
        ...
        # flag to check if line has been detected
        self.detected = False
...
def get_fits_by_sliding_windows(birdeye_binary, line_lt, line_rt, n_windows = 9, verbose = False):
    ...
    # Taking the left side of the histogram
    leftx_base = np.argmax(histogram[:midpoint])
    # Taking the right side of the histogram
    rightx_base = np.argmax(histogram[midpoint: ]) + midpoint

    windows_height = np.int(height / n_windows)
    ...
    for window in range(n_windows):
        win_y_low = height - (window + 1) * windows_height
        win_y_high = height - window * windows_height
    ...
    # TODO - Change this variable name to someting else not plot_y since we're using that somewhere else
